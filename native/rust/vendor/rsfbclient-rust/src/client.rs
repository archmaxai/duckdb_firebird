...
/// How many rows to request from the server per `op_fetch`. Larger batches
/// amortize the network round-trip across many rows, which is essential over
/// high-latency links. Metadata rows are tiny, so this comfortably fits inside
/// the (enlarged) socket read buffer.
const FETCH_BATCH_SIZE: u32 = 200;

/// Size of the per-connection socket read buffer. Made large enough that a full
/// fetch batch typically arrives in a single `read`, so `read_packet` returns
/// on a short read with the whole burst intact.
...
pub struct StmtHandleData {
    ...
    param_count: usize,
    /// Rows already received from the server but not yet handed to the caller.
    /// We fetch in batches to amortize network round-trips; callers still pull
    /// one row at a time via `fetch`.
    pending: std::collections::VecDeque<Vec<Column>>,
    /// Set once the server has signalled end-of-stream for this statement.
    eof: bool,
}
...
impl FirebirdWireConnection {
    ...
    pub fn connect(
        ...
        let socket = connect_with_timeout(host, port)?;

        // Apply read/write timeouts so a stalled relay surfaces as an error
        // instead of blocking forever. Overridable via FIREBIRD_SOCKET_TIMEOUT_SECS
        // (0 disables timeouts).
        let io_timeout = env::var("FIREBIRD_SOCKET_TIMEOUT_SECS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(60);
        if io_timeout > 0 {
            let d = Some(Duration::from_secs(io_timeout));
            let _ = socket.set_read_timeout(d);
            let _ = socket.set_write_timeout(d);
        }
        ...
        // Large enough that a full fetch batch usually arrives in one read.
        let mut buff = vec![0; READ_BUFFER_LENGTH].into_boxed_slice();
    ...
    pub fn execute(
        ...
        // A fresh execution opens a new cursor: drop any rows buffered from a
        // previous run of this statement.
        stmt_handle.pending.clear();
        stmt_handle.eof = false;
    ...
    /// Fetch the next row from the executed statement, coercing the types
    /// according to the provided blr.
    ///
    /// Internally this fetches rows from the server in batches (see
    /// [`FETCH_BATCH_SIZE`]) and buffers them on the statement, so the common
    /// row-at-a-time iterator pattern only pays one network round-trip per
    ...
    pub fn fetch(
        ...
        if let Some(row) = stmt_handle.pending.pop_front() {
            return Ok(Some(row));
        }
        if stmt_handle.eof {
            return Ok(None);
        ...
        self.fetch_batch(tr_handle, stmt_handle)?;

        Ok(stmt_handle.pending.pop_front())
    }

    /// Request a batch of rows from the server and store them on the statement.
    ///
    /// The Firebird wire protocol does not length-prefix the overall fetch
    /// burst, and a multi-row burst can be split arbitrarily across TCP reads.
    /// We therefore accumulate bytes and re-attempt parsing the whole batch
    /// (which is side-effect free: blob bodies are only fetched later in
    /// `into_column`) until it is complete, reading more from the socket on
    /// demand.
    fn fetch_batch(
        &mut self,
        tr_handle: &mut TrHandle,
        ...
        let (rows, eof, lazy_consumed) = loop {
            let n = self.socket.read(&mut self.buff)?;
            if n == 0 {
                return Err("Connection closed by server while fetching".into());
            }
            acc.put_slice(&self.buff[..n]);

            match parse_fetch_batch(
                acc.clone().freeze(),
                &stmt_handle.xsqlda,
                self.version,
                &self.charset,
                ...
                BatchParse::NeedMore => continue,
                BatchParse::Done {
                    rows,
                    eof,
                    lazy_consumed,
                } => break (rows, eof, lazy_consumed),
            }
        };

        self.lazy_count -= lazy_consumed;

        for parsed_cols in rows {
            let mut cols = Vec::with_capacity(parsed_cols.len());
            for pc in parsed_cols {
                cols.push(pc.into_column(self, tr_handle)?);
            }
            stmt_handle.pending.push_back(cols);
        }

        if eof {
            stmt_handle.eof = true;
        }
...
/// Outcome of attempting to parse a whole fetch batch from a (possibly
/// incomplete) buffer.
enum BatchParse {
    /// The buffer does not yet contain the full batch; read more and retry.
    NeedMore,
    /// The batch was parsed successfully.
    Done {
        rows: Vec<Vec<ParsedColumn>>,
        eof: bool,
        lazy_consumed: u32,
    },
}

/// Parse an entire fetch batch (prelude + up to `max_rows` row records, or an
/// end-of-stream marker) from `view`.
///
/// This is side-effect free: it only consumes bytes from the (owned, cheaply
/// cloned) `view` and returns owned `ParsedColumn`s. Blob bodies are fetched
/// separately afterwards. That makes it safe to call repeatedly on a growing
/// buffer until it succeeds. A truncated row record yields [`BatchParse::NeedMore`].
fn parse_fetch_batch(
    mut view: Bytes,
    xsqlda: &[XSqlVar],
    ...
    lazy_count: u32,
    max_rows: u32,
) -> Result<BatchParse, FbError> {
    macro_rules! ensure {
        ($n:expr) => {
            if view.remaining() < $n {
                return Ok(BatchParse::NeedMore);
            }
        };
    }

    // Read the next op code, skipping any dummy packets.
    macro_rules! next_op {
        () => {{
            ensure!(4);
            let mut oc = view.get_u32()?;
            while oc == WireOp::Dummy as u32 {
                ensure!(4);
                oc = view.get_u32()?;
            }
            oc
    ...
    let mut op_code = next_op!();

    let mut lazy_consumed = 0u32;
    // Lazy success responses queued by a prior free_statement. These are small
    // and always arrive in the first read, so propagate parse errors directly.
    for _ in 0..lazy_count {
        if op_code != WireOp::Response as u32 {
            return err_conn_rejected(op_code);
        }
        parse_response(&mut view)?;
        lazy_consumed += 1;
        op_code = next_op!();
    }

    if op_code == WireOp::Response as u32 {
        // Server returned an error response instead of rows.
        parse_response(&mut view)?;
        // parse_response returns Ok only if there was no error status; treat as
        // an empty, finished result in that unlikely case.
        return Ok(BatchParse::Done {
            rows: Vec::new(),
            eof: true,
            lazy_consumed,
        });
    ...
    loop {
        // `op_code` is FetchResponse here. parse_fetch_response reads the status
        // word and, for a row, the row data. A truncated record means we need
        // to read more bytes from the socket.
        match parse_fetch_response(&mut view, xsqlda, version, charset) {
            ...
            Err(_) => return Ok(BatchParse::NeedMore),
        }
...
/// Resolve `host:port` and connect with a bounded timeout, trying each resolved
/// address in turn. Overridable via FIREBIRD_CONNECT_TIMEOUT_SECS (default 15s).
fn connect_with_timeout(host: &str, port: u16) -> Result<TcpStream, FbError> {
    let connect_timeout = env::var("FIREBIRD_CONNECT_TIMEOUT_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(15);
    ...
    let addrs: Vec<_> = (host, port)
        .to_socket_addrs()
        .map_err(|e| FbError::from(format!("failed to resolve '{}:{}': {}", host, port, e)))?
        .collect();
