...
/// Fetch row request
pub fn fetch(stmt_handle: u32, blr: &[u8], message_count: u32) -> Bytes {
    ...
    // Message count: how many rows the server may stream back in a single
    // response burst. Upstream hard-coded this to 1, costing a full network
    // round-trip per row. Batching is the key to usable throughput over
    // high-latency links.
    req.put_u32(message_count.max(1));
